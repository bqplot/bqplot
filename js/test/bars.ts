...
import {create_model, create_model_bqplot, create_view, create_figure_bars} from './widget-utils'
...
describe("bars >", () => {
    ...
    it("create 1d", async function() {
        ...
        var heights = bars.d3el.selectAll(".bargroup rect.bar")[0].map((el) => Number(el.getAttribute('height')));
    ...
    it("create 2d classic", async function() {
        ...
        d3.timer.flush() // this makes sure the animations are all executed
        var heights = bars.d3el.selectAll(".bargroup rect.bar")[0].map((el) => Math.round(Number(el.getAttribute('height'))));
        var width = objects.figure.plotarea_width;
        var height = objects.figure.plotarea_height;
