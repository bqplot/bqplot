...
function GLAttributes(names, names_vec3, getter, sequence_index, names_vec4, defaults) {
    ...
    this.compute_length = function(new_length) {
        this.length = Infinity;
        _.each(this.array,      (ar, name) => this.length = Math.min(this.length, ar.length))
        _.each(this.array_vec3, (ar, name) => this.length = Math.min(this.length, ar.length))
    }
...
module.exports = {GLAttributes: GLAttributes}