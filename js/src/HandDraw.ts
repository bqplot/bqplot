...
export class HandDraw extends Interaction {
  ...
  mousedown() {
    ...
    this.d3el.on('mousemove', () => {
      this.mousemove();
    });
    this.d3el.on('mouseleave', () => {
      this.mouseup();
    });
    this.d3el.on('mouseup', () => {
      this.mouseup();
    });
  }
  ...
  mouse_entry(memory) {
    ...
    if (this.active) {
      ...
      for (let i = min; i <= max; ++i) {
        if (
          (!this.valid_min || lines_model.x_data[xindex][i] >= this.min_x) &&
          (!this.valid_max || lines_model.x_data[xindex][i] <= this.max_x)
        ) {
      ...
      this.lines_view.d3el
        .select('#curve' + (this.line_index + 1))
        .attr('d', (d) => {
  ...
  capnfloor(val) {
    ...
    // and max_x
    return Math.max(
      Math.min(val, this.model.get('max_x')),
  ...
  set_limits() {
    ...
    if (is_date) {
      ...
      this.valid_min = !(this.min_x === null || this.min_x === undefined);
      this.valid_max = !(this.max_x === null || this.max_x === undefined);
    }
  }
