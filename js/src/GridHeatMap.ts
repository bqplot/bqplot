...
export class GridHeatMap extends Mark {
  ...
  set_positional_scales() {
    const x_scale = this.scales.column,
      y_scale = this.scales.row;
  ...
  expand_scale_domain(scale, data, mode, start) {
    ...
    let min_diff;
    if (mode === 'expand_one') {
      current_pixels = data.map((el) => {
        return scale.scale(el);
      });
      const diffs = current_pixels.slice(1).map((el, index) => {
      ...
      min_diff = d3.min(
        current_pixels.slice(1).map((el, index) => {
          return el - current_pixels[index];
        })
      );
  ...
  click_handler(args) {
    ...
    if (elem_index > -1 && accelKey) {
      ...
      if (d3GetEvent().shiftKey) {
        ...
        const row_index =
          selected.length !== 0 ? that.anchor_cell_index[0] : row;
        const col_index =
          selected.length !== 0 ? that.anchor_cell_index[1] : column;
  ...
  set_style_on_elements(style, indices, elements) {
    ...
    elements =
      !elements || elements.length === 0
        ? this._filter_cells_by_cell_num(this._cell_nums_from_indices(indices))
        : elements;
  ...
  clear_style(style_dict, indices?, elements?) {
    ...
    if (!elements || elements.length === 0) {
      if (indices) {
        elements = this._filter_cells_by_cell_num(
          this._cell_nums_from_indices(indices)
        );
      } else {
  ...
  _filter_cells_by_cell_num(
    ...
    return this.display_cells.filter((el) => {
  ...
  apply_styles() {
    ...
    const selected_cell_nums = this._cell_nums_from_indices(
    ...
    if (
      this.anchor_cell_index !== null &&
      this.anchor_cell_index !== undefined
    ) {
  ...
  selector_changed(point_selector, rect_selector) {
    ...
    const that = this;
    const sel_cols = _.filter(col_indices, (index) => {
      return rect_selector([that.column_pixels[index], []]);
    });
    const sel_rows = _.filter(row_indices, (index) => {
      return rect_selector([[], that.row_pixels[index]]);
    });
    let selected = sel_cols.map((s) => {
      return sel_rows.map((r) => {
        return [r, s];
      });
    });
    // @ts-ignore
  ...
  draw() {
    ...
    if (
      this.model.modes.row !== 'middle' &&
      this.model.modes.row !== 'boundaries'
    ) {
      new_domain = this.expand_scale_domain(
        row_scale,
        this.model.rows,
      ...
      if (
        d3.min(new_domain) < d3.min(row_scale.model.domain) ||
        d3.max(new_domain) > d3.max(row_scale.model.domain)
      ) {
        // Update domain if domain has changed
        row_scale.model.compute_and_set_domain(
          new_domain,
          row_scale.model.model_id
        );
      }
    }

    if (
      this.model.modes.column !== 'middle' &&
      this.model.modes.column !== 'boundaries'
    ) {
      new_domain = this.expand_scale_domain(
        column_scale,
        this.model.columns,
      ...
      if (
        d3.min(new_domain) < d3.min(column_scale.model.domain) ||
        d3.max(new_domain) > d3.max(column_scale.model.domain)
      ) {
        // Update domain if domain has changed
        column_scale.model.compute_and_set_domain(
          new_domain,
          column_scale.model.model_id
        );
      }
    }

    const row_plot_data = this.get_tile_plotting_data(
      row_scale,
      this.model.rows,
      this.model.modes.row,
      row_start_aligned
    );
    const column_plot_data = this.get_tile_plotting_data(
      column_scale,
      this.model.columns,
      this.model.modes.column,
      col_start_aligned
    ...
    this.row_pixels = row_plot_data.start.map((d, i) => {
      return [d, d + row_plot_data.widths[i]];
    });
    this.column_pixels = column_plot_data.start.map((d, i) => {
      return [d, d + column_plot_data.widths[i]];
    });

    this.display_rows = this.d3el
    ...
    const data_array = row_nums.map((row) => {
      return col_nums.map((col) => {
        return that.model.mark_data[row * num_cols + col];
      });
    });

    this.display_cells = this.display_rows
    ...
    this.display_cells
      ...
      .attr('y', 0)
      .attr('width', (d, i) => {
        return column_plot_data.widths[i];
      })
      .attr('height', (d) => {
        return row_plot_data.widths[d.row_num];
      });

    // cell labels
    this.display_cell_labels = this.display_rows
      ...
      .attr('x', (d, i) => {
        return column_plot_data.start[i] + column_plot_data.widths[i] / 2;
      })
      .attr('y', (d) => {
        return row_plot_data.widths[d.row_num] / 2;
      })
    ...
    this.display_cells.on('click', (d, i) => {
  ...
  update_labels() {
    ...
    let fonts = this.d3el.selectAll('.heatmapcell_label').text((d: any, i) => {
      return display_format ? display_format(d.color) : null;
    });
  ...
  get_tile_plotting_data(scale, data, mode, start) {
    ...
    if (mode === 'middle') {
      start_points = data.map((d) => {
        return scale.scale(d);
      });
      widths = data.map((d) => {
        return scale.scale.bandwidth();
      });

      return { start: start_points, widths: widths };
    }
    if (mode === 'boundaries') {
      const pixel_points = data.map((d) => {
        return scale.scale(d);
      });
      widths = [];
      ...
      start_points =
        pixel_points[1] > pixel_points[0]
          ? pixel_points.slice(0, -1)
          : pixel_points.slice(1);
      return {
        start: start_points,
        widths: widths,
      };
    ...
    if (mode === 'expand_one') {
      ...
      if (start) {
        ...
        widths = start_points.slice(1).map((d, ind) => {
        ...
        if (start_points[0] < start_points[1]) {
          widths = Array.prototype.concat(widths, [
            Math.abs(bounds - d3.max(start_points)),
          ]);
        } else {
        ...
        if (start_points[1] > start_points[0]) {
          ...
          widths = Array.prototype.concat(widths, [
            Math.abs(bounds - start_points.slice(-1)[0]),
          ]);
