...
export class OrdinalScale extends Scale {
  ...
  expand_domain(old_range, new_range) {
    ...
    const outer_padding =
      unpadded_scale.range().length > 0
        ? Math.abs((new_range[1] - old_range[1]) / unpadded_scale.bandwidth())
        : 0;
  ...
  invert(pixel) {
    ...
    const pixel_vals = domain.map((d) => {
      return that.scale(d) + that.scale.bandwidth() / 2;
    });
    const abs_diff = pixel_vals.map((d) => {
  ...
  invert_range(pixels) {
    ...
    const filtered_ind = indices.filter((ind) => {
      return pixel_vals[ind] >= pixels[0] && pixel_vals[ind] <= pixels[1];
    });
    return filtered_ind.map((ind) => {
      return domain[ind];
    });
  }
