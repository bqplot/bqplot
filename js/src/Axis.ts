...
export class Axis extends WidgetView {
    ...
    compute_tick_values (num_ticks) {
        ...
        if (this.axis_scale.model.type === 'date' ||
            ...
            const epoch_times = scale_domain.map((date) => date.getTime());
            const step = (scale_domain[scale_domain.length - 1] - scale_domain[0]) / (num_ticks - 1);

            const range_in_times = _.range(epoch_times[0], epoch_times[scale_domain.length - 1] + (step * 0.5), step);
        ...
        if (this.axis_scale.model.type === 'ordinal') {
            ...
            const indices = _.range(0, scale_domain.length, scale_domain.length / num_ticks);
