...
describe('scatter mega >', () => {
  ...
  it('colors', async function () {
    ...
    expect(
      scatter.scatter_material.uniforms['domain_color'].value
    ).to.deep.equal([new_color[3], new_color[1]]);

    const new_color2 = new Float32Array([0.5, 2, 0.6, -0.3]);
    scatter.model.set('color', new_color2);
    expect(
      scatter.scatter_material.uniforms['domain_color'].value
    ).to.deep.equal([new_color2[3], new_color2[1]]);
  ...
  it('selection', async function () {
    ...
    expect(scatter.scatter_material.uniforms['has_selection'].value).to.be
      .false;
