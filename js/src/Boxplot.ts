...
class Boxplot extends Mark {
    ...
    create_listeners() {
        ...
        this.listenTo(this.model, "change:outlier_fill_color", this.updateOutlierFillColor);
    ...
    private updateOpacities() {
        this.d3el.selectAll(".boxplot").style("opacity", (d, i: number) => this.get_mark_opacity(d, i));

        if (this.legendEl) {
            this.legendEl.selectAll("path").attr("opacity", (d, i: number) => this.get_mark_opacity(d, i));
        }
    }
    ...
    selector_changed(point_selector, rect_selector) {
        ...
        const selected = indices.filter(index => rect_selector(this.pixelCoords[index]));
    ...
    invert_point(pixel) {
        ...
        const absDiff = this.xPixels.map(function(elem) { return Math.abs(elem - pixel); });
        ...
        for(let i = 0; i < this.model.mark_data.length; ++i) {
            ...
            for (let j=0; j < values[1].length; ++j)  {
               ...
               if (autoDetectOutliers && (plotY > lowerBound || plotY  < upperBound)) {
                   this.outlierData.push({x: this.scales.x.scale(values[0]),
                                          y: plotY});
               } else {
            ...
            this.plotData.push({x, boxLower, boxMedian, boxUpper, whiskerMin, whiskerMax, dataDict});
        }
    }
    ...
    draw() {
        ...
        this.xPixels = this.model.mark_data.map((el) => this.scales.x.scale(el[0]) + this.scales.x.offset);

        const width = this.getBoxWidth() / 2;
        this.pixelCoords = this.plotData.map(function(d) {
    ...
    private drawMarkPaths(parentClass, selector) {
        ...
        // Create new
        const newBoxplots = boxplot.enter()
            .append("g")
            .attr ("class", "boxplot")
        ...
        selector.selectAll(".boxplot").data(this.plotData)
            .style("stroke", this.model.get("stroke"))
            .style("opacity", color)
        ...
        selector.selectAll(".box").data(this.plotData)
        ...
        //Median line
        selector.selectAll(".median_line").data(this.plotData)
            .style("stroke-width", 2)
            .attr("d", (d: BoxPlotData) => {
                const x = 0;
                const medianY = d.boxMedian;

                return "M" + (x - width / 2) + "," + medianY + " L" + (x + width / 2) + "," +  medianY;
            });

        //Max and Min Whiskers
        //Max to top of the Box
        selector.selectAll(".whisker_max").data(this.plotData)
            .attr("d", (d: BoxPlotData) => {
              const x = 0;
        ...
        selector.selectAll(".whisker_max_end").data(this.plotData)
            .attr("d", (d: BoxPlotData) => {
                const x = 0;
        ...
        //Max to top of the Box
        selector.selectAll(".whisker_min").data(this.plotData)
            .attr("d", (d: BoxPlotData) => {
                const x = 0;
                // The price points are sorted, so 1st element is min
        ...
        selector.selectAll(".whisker_min_end").data(this.plotData)
            .attr("d", (d: BoxPlotData) => {
                const x = 0;
                // The price points are sorted, so 1st element is min
                const minY = d.whiskerMin;
        ...
        // Set outlier data
        selector.selectAll(".outlier").data(this.outlierData)
            .style("fill", this.model.get("outlier_fill_color"))
    ...
    draw_legend(elem, x_disp, y_disp, inter_x_disp, inter_y_disp) {
        ...
        const leg = this.legendEl.enter().append("g")
            .attr("transform", function(d, i) {
