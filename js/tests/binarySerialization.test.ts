...
describe("Binary serialization", () => {
    ...
    test("Test .generateMultiIndexArrayLocations()", async () => {
        ...
        expect([...deserialized_x]).toEqual([0, 0.5, 1.0]);

        const serialized_x = array_or_json.serialize(deserialized_x, null);
        expect(serialized_x).toEqual(x);
    });

    test("deserialize/serialize date arrays", async () => {
        ...
        const deserialized_x = array_or_json.deserialize(x, null);
        expect(deserialized_x.type).toEqual('date');
        expect([...deserialized_x]).toEqual([1109289600000, 1109376000000, 1109462400000]);

        const serialized_x = array_or_json.serialize(deserialized_x, null);
        expect(serialized_x).toEqual(x);
    });

    test("deserialize/serialize string arrays", async () => {
        ...
        expect(deserialized_strlist).toEqual(strlist);

        const serialized_strlist = array_or_json.serialize(deserialized_strlist, null);
        expect(serialized_strlist).toEqual(strlist);
    });

    test("deserialize/serialize nested arrays", async () => {
        const x = [
        ...
        const deserialized_x = array_or_json.deserialize(x, null);
        expect(deserialized_x.length).toEqual(3);
        expect([...deserialized_x[0]]).toEqual([0, 0.5, 1.0]);
        expect([...deserialized_x[1]]).toEqual([0, 0.5]);
        expect([...deserialized_x[2]]).toEqual([0, 0.5, 2., 3.]);
