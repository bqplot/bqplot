    ...
    var OHLC = MarkViewModule.Mark.extend({
        ...
        draw_mark_paths: function(type, min_x_difference, selector, dat) {
            ...
            for(var i = 0; i < dat.length; i++) {
                ...
                if(px.high === -1 || px.low === -1) {
                    high[i] = open[i];
                    low[i] = close[i];
                } else {
                    high[i] = y_scale.scale(dat[i][px.high]);
                    low[i] = y_scale.scale(dat[i][px.low]);
                }
            ...
            // a bar.
            if(type == "candle" && px.open !== -1 && px.close !== -1) {
                /*
                 *      | <-------- head
                 *  ---------
                 *  |       |
