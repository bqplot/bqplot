...
@register_mark('bqplot.MarkerLines')
class MarkerLines(Mark):

    """Marker Lines mark.

    Represents a marker line (line with markers to highlight x and y values)
    ...
    line_style: {'solid', 'dashed', 'dotted'}
        Line style
    marker: List of marker shapes belonging to enum
    ...
    # Scaled attributes
    x = NdArray(sync=True, min_dim=1, max_dim=2, scaled=True, rtype='Number',
                atype='bqplot.Axis')
    y = NdArray(sync=True, min_dim=1, max_dim=2, scaled=True, rtype='Number',
                atype='bqplot.Axis')
    color = NdArray(None, sync=True, allow_none=True,
                    scaled=True, rtype='Color', atype='bqplot.ColorAxis',
                    min_dim=1, max_dim=1)
    ...
    _view_name = Unicode('MarkerLines', sync=True)
    _view_module = Unicode('nbextensions/bqplot/MarkerLines', sync=True)
    _model_name = Unicode('LinesModel', sync=True)
    _model_module = Unicode('nbextensions/bqplot/LinesModel', sync=True)
